...
const int base_lidar_dist = 200;   //distance from vehicle centre to lidar in cm(only in y-direction)
...
const int range = 16/map_res;//10m in cells
...
pair<float,float> convToCart(float i,float r) //convert from polar to cartesian co-ordinates
{
   float ang = i*0.25*DEG_TO_RAD;
   float x = r*cos(ang +(curr_head-starting_head)*DEG_TO_RAD);
   float y = -1*r*sin(ang+(curr_head-starting_head)*DEG_TO_RAD);
   ...
   for(i=l_height-1;i>=0;i--)//initialising map
   {
      for(j=0;j<l_width;j++)
      {
         ...
         if(lane_map.data[i1*l_width+j]==2){ 
            ...
            double dist = sqrt(i1*i1*l_res*l_res+(j-l_width/2)*(j-l_width/2)*l_res*l_res);
            double ang_lane =  atan2(i1,(j-l_width/2));
            vector< pair<float,float> > lane_cartesian(1,make_pair(0,0));
            lane_cartesian[0] = convToCart(4*ang_lane*RAD_TO_DEG-(curr_head-starting_head)*0 - 360,dist);
            int x_coord_lane = round(lane_cartesian[0].first/map_res);
            int y_coord_lane = round(lane_cartesian[0].second/map_res); 
            lane_map_new.data[x_coord_lane*l_width_new+y_coord_lane+l_width_new/2] = 2; 
