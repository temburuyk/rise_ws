...
void leftimage(const sensor_msgs::ImageConstPtr& original_image)
{
	...
	roi = src_left(Rect(q[0].x - 1, q[2].y - 1, 70, 61));
	warpPerspective(src, roi, transform, Size(occ_grid_width, occ_grid_height));

	for (int i = 0; i < Lane_points_left.size(); ++i)
	{
		Lane_points_left[i].clear();
	}

	Mat LeftROI;
	roi.copyTo(LeftROI);
	//imshow("LeftROI", LeftROI);
...
void middleimage(const sensor_msgs::ImageConstPtr& original_image)
{
	...
	Point2f p[4] = {Point(0, 0), Point(639,0), Point(639, 479), Point(0, 479)};
	Point2f q[4] = {Point(187, 287), Point(299, 279), Point(264, 379), Point(233, 381)};
...
void rightimage(const sensor_msgs::ImageConstPtr& original_image)
{
	...
	roi = src_right(Rect(q[1].x - 1, q[0].y - 1, 82, 53));

	warpPerspective(src, roi, transform, Size(occ_grid_width, occ_grid_height));

	for (int i = 0; i < Lane_points_right.size(); ++i)
	{
		Lane_points_right[i].clear();
	}

	Mat RightROI;
	roi.copyTo(RightROI);
	//imshow("RightROI", RightROI);
