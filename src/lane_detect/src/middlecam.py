...
class image_converter:
  ...
  def __init__(self):
    self.image_pub = rospy.Publisher("/camera/image_raw2",Image, queue_size=0)

    self.bridge = CvBridge()
    self.image_sub = rospy.Subscriber("/usb_cam/image_raw",Image,self.callback) #/usb_cam/image_raw
  def callback(self,data):
    ...
    thgauss = cv2.adaptiveThreshold(img,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,\
         cv2.THRESH_BINARY,(2*60 + 3),72-100)
    cv2.rectangle(im1,(240,330),(400,480),0,-1)#CV_FILLED=-1
    cv2.imshow('gaussian adaprive threshold',thgauss)
    ...
    #cv2.imshow('im1',im1)
    cv2.imshow('Contour area thresholding',final)
    cv2.waitKey(3)
