...
namespace bof {
    ...
    void Cell::getAntecedents(std::set<Cell *>& antecedents, std::vector<std::vector<Cell> >& prevOccGrid,int m,int n) {
        ...
        /*    int x = round(xpos + m * dt);
            int y = round(ypos + n * dt);
                if(m>0||(n>0))
            {
                if (y >= 0 && y < prevOccGrid.size()) {
                        if (x >= 0 && x < prevOccGrid[y].size()) {
    ...
    void Cell::updateDistributions(std::vector<std::vector<Cell> >& prevOccGrid,int m,int n) {
        std::set<Cell *> antecedents;
        getAntecedents(antecedents, prevOccGrid,m,n);
    ...
    */
        //
            xVelocityKeys.push_back(m);
            std::vector<float> betaOccRow;
            std::vector<float> betaEmpRow;
               if (!yVelocityKeysUpdated)
                    yVelocityKeys.push_back(n);

                float alphaO = 0;
                float alphaE = 0;
                getPrediction(alphaO, alphaE, m, n, antecedents, prevOccGrid);
