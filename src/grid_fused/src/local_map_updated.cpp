...
/*   for(i=0;i<height;i++)
 ...
 /*  int n = sizeof(path.poses);
   cout<<n<<endl;
   if(n != 0)
   {
for (int z=0; z<n;z++)
{
   cout<<"path_local_map"<<endl;
   path_local_map.at<Vec3b>(int(path.poses[z].pose.position.x),int(path.poses[z].pose.position.y))[2]=255;
...
void path_show(const nav_msgs::Path::ConstPtr& msg1){
cout<<"entered path_show"<<endl;
path=(*msg1);           
int n = sizeof(path.poses);
cout<<n<<endl;
cout<<"defined path"<<endl;
cout<<int(path.poses[2].pose.position.x)<<endl;
/*for (int z=0; z<n;z++)
{
   cout<<"entered for loop"<<endl;
   //path_local_map.at<Vec3b>(int(msg1->poses[z].pose.position.x),int(msg1->poses[z].pose.position.y))[2]=255;
     cout<<int(msg1->poses[z].pose.position.x)<<" "<<int(msg1->poses[z].pose.position.y)<<endl;  
}*/
cout<<"for out"<<endl;
imshow("path_local_map",path_local_map);
cv::waitKey(1);
...
int main(int argc,char** argv)
{
   ...
   ros::Subscriber path_sub=n4.subscribe<nav_msgs::Path>("/path", 1,path_show);
...
map_pub=n2.advertise<nav_msgs::OccupancyGrid>("map",1);
