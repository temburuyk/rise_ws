...
namespace bof {
    ...
    private:
        void getAntecedents(std::set<Cell *>& antecedents, std::vector<std::vector<Cell> >& prevOccGrid,int m,int n);
    ...
    public:
        ...
        void updateDistributions(std::vector<std::vector<Cell> >& prevOccGrid,int m, int n);
        void toString();
    };
}
